...
def aws_cost_explorer_pull():  # noqa: C901
    ...
    while True:
        ...
        if not token:
            break
    # print(results)
    account_id = results[0]["Groups"][0]["Keys"][0]
    ...
    service_and_usage_type = {}
    services = {}
    for result_by_time in results:
        for group in result_by_time["Groups"]:
            amount = group["Metrics"]["UsageQuantity"]["Amount"]
            # unit = group["Metrics"]["UsageQuantity"]["Unit"]
            service_and_usage_type[group["Keys"][1]] = group["Keys"][0]
            services[group["Keys"][0]] = 0
    ...
    for result_by_time in results:
        for group in result_by_time["Groups"]:
            ...
            service_name = service_and_usage_type[usage_type]
            ...
            dimensions = {}
            dimensions["usage_type"] = usage_type
            if float(amount) > 0:
